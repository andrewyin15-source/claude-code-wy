...
基于 Claude Code 泄露源码二开的**本地可运行版本**，支持接入任意 Anthropic 兼容 API（如 MiniMax、OpenRouter 等），并启用全部隐藏功能。

> 原始泄露源码无法直接运行。本仓库修复了启动链路中的多个阻塞问题，并启用所有隐藏功能，使完整的 Ink TUI 交互界面和完整特性集可以在本地工作。
...
**注意**：HISTORY_SNIP 和 UDS_INBOX 由于兼容性问题已排除。
...
| 脚本 | 说明 |
|------|------|
| `enable-full.cjs` | 启用全部 24 个隐藏功能（推荐，排除 HISTORY_SNIP 和 UDS_INBOX） |
| `enable-core.cjs` | 仅启用核心功能（BUDDY, PROACTIVE, COORDINATOR_MODE） |
| `enable-safe.cjs` | 启用除问题功能外的所有功能 |

### 功能发现过程

通过二分查找和组合测试，发现 `UDS_INBOX` 与其他功能组合时会导致 TUI 崩溃，`HISTORY_SNIP` 会导致 API 调用错误。
